...
leyendo los CSVs de presentation_extra_eda.py. No recalcula nada.
"""
import matplotlib
matplotlib.use("Agg")
...
campana = pd.read_csv("price_position_bought.csv")
...
plt.tight_layout()
plt.savefig("price_position_bought.png", dpi=150)
plt.close()

# 2) boxplots original vs z-score
norm = pd.read_csv("normalizacion_boxplot.csv")
...
plt.tight_layout()
plt.savefig("normalizacion_boxplot.png", dpi=150)
plt.close()

# 3) % bought por tag de reputacion
tag = pd.read_csv("reputation_tag_bought.csv")
...
plt.tight_layout()
plt.savefig("reputation_tag_bought.png", dpi=150)
plt.close()
