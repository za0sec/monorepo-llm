...
**Actualización desde Ejercicio 2** (`ejercicio2/Experimentos.md`, sección "Interpretabilidad"): esta misma señal de reputación **no está solo en `title`** — `description` la repite con otra frase al final (ej. tag `"Best Seller"` en `title` ↔ frase "Frequently reordered by returning customers" al final de `description`, ambas con ~65% de `bought`). Se descubrió corriendo un chequeo de interpretabilidad sobre el modelo entrenado: sacar el tag de `title` solo no bajaba el PR-AUC porque `description` mantenía la misma información. Implicancia para el preprocesamiento de texto: es una razón más para no parsear esta señal a mano — está duplicada en dos lugares del texto, con variación en la frase exacta, así que dejarla como texto crudo para que la aprenda el Transformer es más robusto que intentar extraerla con una regla fija.
